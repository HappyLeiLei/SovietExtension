...
// 自动登录状态。
static std::atomic_bool YMAutoLoginScannerStarted(false);
static std::atomic_bool YMAutoLoginClickSucceeded(false);
static std::atomic_bool YMAutoLoginReturnKeySent(false);
static std::atomic_int YMAutoLoginClickAttemptCount(0);
...
static BOOL YMIsAutoLoginEnabled(void) {
    return YMFeatureAutoLoginEnabled;
}
...
static BOOL YMAutoLoginTrySendReturnKeyNow(void) {
    if (YMAutoLoginReturnKeySent.exchange(true)) {
        return NO;
    }
...
static BOOL YMAutoLoginTryPressLoginButtonNow(void) {
    if (YMAutoLoginClickSucceeded.load()) {
        return YES;
    }
    ...
    for (NSWindow *window in windows) {
        ...
        if (YMAutoLoginTryPressView(contentView, debugItems, 0)) {
            YMAutoLoginClickSucceeded.store(true);
            return YES;
        }
...
static void YMAutoLoginScheduleLoginClick(void) {
    ...
    int oldAttempt = YMAutoLoginClickAttemptCount.fetch_add(1);
    if (oldAttempt >= 12) {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(delay * NSEC_PER_SEC)),
        ...
        if (!YMIsAutoLoginEnabled() || YMAutoLoginClickSucceeded.load()) {
            return;
        }
        ...
        if (!ok && !YMAutoLoginClickSucceeded.load() && YMAutoLoginClickAttemptCount.load() < 12) {
...
static void YMStartAutoLoginScanner(void) {
    ...
    if (YMAutoLoginScannerStarted.exchange(true)) {
        YMLog(@"[AutoLogin] scanner already started, skip");
        return;
    }

    YMAutoLoginClickSucceeded.store(false);
    YMAutoLoginReturnKeySent.store(false);
    YMAutoLoginClickAttemptCount.store(0);
...
static void YMLoadFeatureSwitchesFromDefaults(void) {
    ...
    YMLog(@"feature switches antiUpdate=%@ antiRevoke=%@ groupExit=%@ systemWeb=%@ autoLogin=%@",
